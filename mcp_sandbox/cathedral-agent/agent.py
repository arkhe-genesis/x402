# Modificações parciais integradas.
# Importação da biblioteca e código fonte base não foram fornecidos no prompt.
# Abaixo representamos a inserção dentro do método da classe que processa ferramentas.

class SubagentAgent:
    def __init__(self, mcp, console):
        self.mcp = mcp
    ...
    def handle_manual(self, user_input):
        if "spawn" in user_input and "subagent" in user_input:
