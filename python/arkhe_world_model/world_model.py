...
class WorldModelEmbryo(nn.Module):
    ...
    def _get_active_stages(self) -> List[DevelopmentStage]:
        ...
        if self.maturity == MaturityLevel.EMBRYO:
            ...
            stages.remove(DevelopmentStage.TOKEN_GROUNDING)
            stages.remove(DevelopmentStage.EMBODIED_SIMULATION)
            stages.remove(DevelopmentStage.CAUSAL_REASONING)
            return stages
    ...
    def forward(
        ...
        llm_emb = torch.randn(1, self.config.d_model) # STUB
        ...
        if DevelopmentStage.MULTIMODAL_FUSION in self.active_stages:
            if visual_input is not None:
                 visual_input = torch.tensor(visual_input).float()
        ...
        world_emb = torch.randn(1, self.config.state_dim) # STUB
